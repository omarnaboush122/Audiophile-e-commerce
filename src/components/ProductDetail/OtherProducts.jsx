...
const OtherProducts = ({ others }) => {
  ...
  const otherProductsElements = others.map((item, i) => (
    <article key={i} className="flex flex-col gap-8">
      ...
      <Link to={item.link} className=" bg-burntSienna text-white text-sm font-bold w-fit mx-auto px-8 py-4 flex justify-center items-center uppercase tracking-wider transition-colors duration-300 hover:bg-peach">see product</Link>
    </article>
  ))
  return (
